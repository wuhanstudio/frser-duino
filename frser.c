...
const char ca_bitmap[33] = { S_ACK, FRSER_BM_B0, FRSER_BM_B1, FRSER_BM_B2, FRSER_BM_B3, 0 };
const char ca_pgmname[17] = "\x06" FRSER_NAME; /* Small hack to include S_ACK in the name. */
const char ca_serbuf[3] = { S_ACK, (UART_RBUFLEN)&0xFF, (UART_RBUFLEN>>8)&0xFF };
const char ca_syncnop[2] = { S_NAK, S_ACK };

const char ca_opbufsz[3] = { S_ACK, S_OPBUFLEN&0xFF, (S_OPBUFLEN>>8)&0xFF };
#ifndef DYN_WRNMAX
const char ca_wrnlen[4] = { S_ACK, WRNMAX&0xFF, (WRNMAX>>8)&0xFF, 0 };
#endif
const char ca_rdnmaxlen[4] = { S_ACK, RDNMAX&0xFF, (RDNMAX>>8)&0xFF, (RDNMAX>>16)&0xFF };

#ifndef FRSER_FEAT_DYNPROTO
const char ca_bustypes[2] = { S_ACK, SUPPORTED_BUSTYPES };
#endif

#ifdef FRSER_FEAT_PARALLEL
const char ca_chipsize[2] = { S_ACK, FRSER_PARALLEL_BITS };
#endif

/* Commands with a const answer cannot have parameters */
const struct constanswer const_table[S_MAXCMD+1] = {
	{ 1, ca_iface },	// NOP
	{ 3, ca_iface },	// IFACE V
...
const uint8_t op2len[S_MAXCMD+1] = {
...
static const uint8_t bit_mask_lut[8] = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80 };
...
static void do_cmd_opbuf_exec(void) {
	...
	for(readptr=0;readptr<opbuf_bytes;) {
		...
		if ((op == OPBUF_POLL)||(op == OPBUF_POLL_DLY)) {
			...
			uint8_t mask = bit_mask_lut[details&7];
			if (details&0x10) {
				/* toggle mode */
